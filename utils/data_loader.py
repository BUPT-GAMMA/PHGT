...
class data_loader:
    def __init__(self, path, args):
        ...
        if args.drop_edge != 0:
            self.drop_edge(args.drop_edge)
        self.hg = self.gen_dgl_graph()
    ...
    @staticmethod
    def return_empty_edge_index_array():
        return [[], []]
    ...
    def load_links(self):
        ...
        """
        links = {'total':0, 'count':Counter(), 'meta':{}, 'data':defaultdict(list), 'edge_index':defaultdict(self.return_empty_edge_index_array)}
        with open(os.path.join(self.path, 'link.dat'), 'r', encoding='utf-8') as f:
            for line in f:
                ...
                links['edge_index'][r_id][0].append(h_id)
                links['edge_index'][r_id][1].append(t_id)
                links['count'][r_id] += 1
                links['total'] += 1
        new_data = {}
    ...
    def drop_edge(self, ratio):
        edge_num_dict = {}
        for e_t in self.links['edge_index'].keys():
            total_len = len(self.links['edge_index'][e_t][0])
            if str(total_len) not in edge_num_dict:
                edge_num_dict[str(total_len)] = [e_t]
            else:
                edge_num_dict[str(total_len)].append(e_t)
        for length in edge_num_dict.keys():
            e_t_src = edge_num_dict[length][0]
            e_t_dst = edge_num_dict[length][1]
            drop_len = int(int(length) * ratio)
            drop_idx = np.random.randint(0, int(length), drop_len)
            drop_idx = np.sort(drop_idx)[::-1]
            for idx in drop_idx:
                if self.links['edge_index'][e_t_src][0][idx]==self.nodes['count'][0]-1 or self.links['edge_index'][e_t_src][1][idx]==self.nodes['count'][0]-1:
                    continue
                self.links['data'][e_t_src][self.links['edge_index'][e_t_src][0][idx], self.links['edge_index'][e_t_src][1][idx]] = 0
                self.links['data'][e_t_dst][self.links['edge_index'][e_t_dst][0][idx], self.links['edge_index'][e_t_dst][1][idx]] = 0
                self.links['edge_index'][e_t_src][0].pop(idx)
                self.links['edge_index'][e_t_src][1].pop(idx)
                self.links['edge_index'][e_t_dst][0].pop(idx)
                self.links['edge_index'][e_t_dst][1].pop(idx)
                self.links['total'] -= 1
                self.links['count'][e_t_src] -= 1
                self.links['count'][e_t_dst] -= 1
            self.links['data'][e_t_src].eliminate_zeros()
            self.links['data'][e_t_dst].eliminate_zeros()

    def gen_dgl_graph(self):
        """
        Generate the corresponding graph with DGL graph data structure.
        :return: A DGL heterogeneous graph
        """
        edge_type_ids = self.links['meta'].keys()
        edge_index_dict = {}
        edge_feature_dict = {}

        for r_id in edge_type_ids:
            r_type = self.links['meta'][r_id]
            edge_key = (str(r_type[0]), str(r_type[0]) + '-' + str(r_type[1]), str(r_type[1]))
            if edge_key in edge_index_dict:
                src_idx = torch.cat((torch.tensor(self.links['edge_index'][r_id][0]),
                                     torch.tensor(self.links['edge_index'][r_id][1])))
                dst_idx = torch.cat((torch.tensor(self.links['edge_index'][r_id][1]),
                                     torch.tensor(self.links['edge_index'][r_id][0])))
                edge_index_dict[edge_key] = [src_idx, dst_idx]
            else:
                edge_index_dict[edge_key] = [torch.tensor(self.links['edge_index'][r_id][0]),
                                             torch.tensor(self.links['edge_index'][r_id][1])]

        for e_t in edge_index_dict.keys():
            src = edge_index_dict[e_t][0] - self.nodes['shift'][int(e_t[0])]
            dst = edge_index_dict[e_t][1] - self.nodes['shift'][int(e_t[2])]
            edge_index_dict[e_t] = (src, dst)

        g = dgl.heterograph(edge_index_dict)

        node_attr = {}
        for n_t in self.nodes['attr'].keys():
            if self.nodes['attr'][n_t] is None:
                node_attr[str(n_t)] = torch.eye(self.nodes['count'][n_t])
                continue
            node_attr[str(n_t)] = torch.tensor(self.nodes['attr'][n_t]).float()

        for ntype in g.ntypes:
            dim = g.num_nodes(ntype=ntype)
            indices = np.vstack((np.arange(dim), np.arange(dim)))
            indices = torch.LongTensor(indices)
            values = torch.FloatTensor(np.ones(dim))
            g.nodes[ntype].data['h'] = torch.sparse.FloatTensor(indices, values, torch.Size([dim, dim])).to_dense()
